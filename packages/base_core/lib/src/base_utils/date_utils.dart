...
class DateUtils {
  ...
  static final REGEX_AM_PM =
      RegExp('^(\\d{1,2}\\s(pm|am)|\\d{1,2}(\\.|:)\\d{2}\\s(pm|am)|\\d{1,2}(\\.|:)\\d{2}(\\.|:)\\d{2}\\s(pm|am))\$');
  static final REGEX_WEEKDAY =
      RegExp('^($regexMonday|$regexTuesday|$regexWednesday|$regexThursday|$regexFriday|$regexSaturday|$regexSunday)\$');
  ...
  static String utcTimeToLocalByTimezone(String utcTimeString, int timeZoneOffsetHours) {
  ...
  static DateTime _parseDate(String s) {
    ...
    var time = DateTime.parse('${dateSplitted.elementAt(2)}-$monthAsInt-${dateSplitted.elementAt(0)} 00:00:00');
    if (setTime != null) {
  ...
  static DateTime _parseAddRem(String s, DateTime now, {bool rem = false}) {
    ...
    for (var i = 1; i < list.length; i += 2) {
      ...
      if (REGEX_YEAR.hasMatch(type)) {
        ...
        now = DateTime(now.year + valueAsInt, now.month, now.day, now.hour, now.minute, now.second);
      } else if (REGEX_MONTH.hasMatch(type)) {
        //print('Add $valueAsInt month(s)');
        now = DateTime(now.year, now.month + valueAsInt, now.day, now.hour, now.minute, now.second);
    ...
    if (setTime != null) {
      if (setTime.contains(':')) {
        ...
        now = DateTime(now.year, now.month, now.day, hhmmss.hour, hhmmss.minute, hhmmss.second);
      } else {
        ...
        for (var i = 1; i < list.length; i += 2) {
          ...
          if (REGEX_HOUR.hasMatch(type)) {
            now = DateTime(now.year, now.month, now.day, valueAsInt, now.minute, now.second);
          } else if (REGEX_MINUTES.hasMatch(type)) {
            now = DateTime(now.year, now.month, now.day, now.hour, valueAsInt, now.second);
          } else if (REGEX_SECONDS.hasMatch(type)) {
            now = DateTime(now.year, now.month, now.day, now.hour, now.minute, valueAsInt);
          }
        }
      }
    }
  ...
  static DateTime _parseSetTime(String setTime, DateTime time) {
    if (setTime.contains(':')) {
      var hhmmss = DateTime.parse('1965-01-01 $setTime');
      return DateTime(time.year, time.month, time.day, hhmmss.hour, hhmmss.minute, hhmmss.second);
      ...
      var hhmmss = DateTime.parse('1965-01-01 ' + setTime);
      return DateTime(time.year, time.month, time.day, hhmmss.hour + baseTime, hhmmss.minute, hhmmss.second);
    } else {
      var list = setTime.split(' ');
      for (var i = 1; i < list.length; i += 2) {
        var value = list.elementAt(i - 1);
        ...
        var type = list.elementAt(i);
        if (REGEX_HOUR.hasMatch(type)) {
          time = DateTime(time.year, time.month, time.day, valueAsInt, time.minute, time.second);
        } else if (REGEX_MINUTES.hasMatch(type)) {
          time = DateTime(time.year, time.month, time.day, time.hour, valueAsInt, time.second);
        } else if (REGEX_SECONDS.hasMatch(type)) {
          time = DateTime(time.year, time.month, time.day, time.hour, time.minute, valueAsInt);
        }
      }
      return time;
    }
